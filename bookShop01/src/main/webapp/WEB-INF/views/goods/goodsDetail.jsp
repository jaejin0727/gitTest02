...
<script type="text/javascript">
	...
	function fn_display_detail(type,review_content){
		if (type == 'open') {
			// 팝업창을 연다.
			jQuery('#layer_review').attr('style', 'visibility:visible');
			$('#message').text(review_content);

			// 페이지를 가리기위한 레이어 영역의 높이를 페이지 전체의 높이와 같게 한다.
			jQuery('#layer_review').height(jQuery(document).height());
		}

		else if (type == 'close') {

			// 팝업창을 닫는다.
			jQuery('#layer_review').attr('style', 'visibility:hidden');
		}
	}
	...
	function fn_showTbl() {
		...
		//$('#btn_review_mod').attr('style', 'visibility:hidden');
	...
	function fn_regReview() {
 	...
 	function fn_review_modify() {
		...
		$('#tbl_review').attr('style', 'visibility:visible');
		$('#btn_review').attr('style', 'visibility:hidden');
		//$('#btn_review_reg').attr('style', 'visibility:hidden'); //화면에서 소멸
...
/*
	function fn_reviewDel() {
		...
		document.body.
		formObj.method="post";
	    formObj.action="/bookshop01/goods/delReview.do";
	    formObj.submit(); 
	}*/
</script>
</head>
<body>
			...
			<form name="frmReview">
				...
				<c:forEach var="review" items="${goodsMap.reviewList }" varStatus="cnt">
					<tr>
						...
						<td><a href="javascript:fn_reviewDetail('open','${review.review_content }','${cnt.count-1 }');"> <!-- 반복횟수=배열순서 -->
					...
					<%-- <input type="hidden" value="${review.review_content }" id="${review.review_id}" /> --%>
				</c:forEach>
				...
				<br><br>
				<input  name="review_write" type="button" value="리뷰쓰기" onClick="fn_showTbl()" id="btn_review" /><br><br>
				...
				<table style="visibility: hidden" id="tbl_review" >
					...
					<tr>
						<td>글내용</td>
						<td>
							<textarea cols="80" rows="10" id="t_review_content" />도서평을 입력하세요.</textarea>
						</td>
					</tr>
					<tr>
						<td>비밀번호</td>
	...
	<div id="layer_review" style="visibility: hidden">
		<div id="popup">
			<!-- 팝업창 닫기 버튼 -->
			<a href="javascript:" onClick="javascript:fn_display_detail('close', '.layer_review');"> <img
				src="${pageContext.request.contextPath}/image/close.png" id="close" />
			</a> <br /> <font size="5" id="contents"><p id="message">글 상세 내용입니다.</p></font><br>
	<form action='${pageContext.request.contextPath}/cart/myCartMain.do'>				
			<input  name="btn_cart_list" type="button" value="닫기" onClick="fn_reviewDetail('close','.layer_review')">
			<input  name="btn_review_modify" type="button" value="수정하기" onClick="fn_review_modify()">
			<input  name="btn_review_del" type="button" value="삭제하기" onClick="fn_reviewDel('close','.layer_review')">
				<div></div>
	</form>			
</body>
</html>
